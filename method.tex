...
\end{equation}
where $T$ is the total number of steps. The state transition $s_{k+1} \leftarrow \text{Env}(s_k, a_k)$ is stochastic, as it depends on the external return values from the MCP tools.
...
\begin{equation}
    \mathbb{E}[\mathcal{J}] =
    \underbrace{\Pr(\tilde{t} \in \tau)}_{\text{Attraction}}
    \cdot
    \underbrace{\mathbb{E}\!\left[\,\mathcal{J}_{\text{man}}(\tau, o_{\text{adv}}) \mid \tilde{t} \in \tau\,\right]}_{\text{Manipulation}}
    \label{eq:expected_decomposition}
\end{equation}
This separates the goal into (i) maximizing the selection probability of the malicious tool, and (ii) maximizing conditional attack effectiveness given selection, exactly aligning with Phase~I (Attraction) and Phase~II (Manipulation).
