...
The primary objective of the attacker is to induce a specific target behavior $y_{\text{guidance}}$ by manipulating the malicious tool configuration $\tilde{t}$. We categorize the potential impact into four critical attack scenarios:
\begin{itemize}
...
We initialize an empty execution record $H_0 = []$.
...
We model the environment transition kernel as $\mathcal{P}: \mathcal{S} \times \mathcal{A} \to \Delta(\mathcal{S})$ and write
\begin{equation}
...
The trace unfolds by alternating policy sampling and environment transition: starting from $s_0 = (q, H_0)$, at each step $k$ we draw $a_k \sim \pi(s_k)$ and then sample $s_{k+1} \sim \mathcal{P}(s_k, a_k)$ until the agent emits a final response or the step budget $K$ is reached.
...
The components are
\begin{equation}
    P_{\text{sel}}(\tilde{t}) = \Pr_{\tau \sim \pi(q,\, \mathcal{T} \cup \{\tilde{t}\})}[\tilde{t} \in \tau]
    \label{eq:psel}
\end{equation}
\begin{equation}
    E_{\text{man}}(\tilde{t}) = \mathbb{E}\!\left[\mathcal{J}_{\text{man}}(\tau, o_{\text{adv}}) \mid \tilde{t} \in \tau\right]
    \label{eq:eman}
\end{equation}
Phase~I tunes $(n, d)$ to maximize Attraction, and Phase~II tunes $o_{\text{adv}}$ to maximize Manipulation given the chosen $(n, d)$.
...
Let $N_{\text{attr}}$ denote the total generation budget allocated for the Attraction phase. We adopt a \textbf{uniform allocation strategy}, assigning $N_s = N_{\text{attr}} / 5$ seeds to each persuasion category. Define the persuasion subset $\mathcal{S} = \{s_{\text{auth}}, s_{\text{urg}}, s_{\text{comp}}, s_{\text{res}}, s_{\text{sec}}\}$; for each $s \in \mathcal{S}$, we prompt a generator LLM $\mathcal{G}$ to sample candidates:
\begin{equation}
(n_i, d_i) \sim \mathcal{G}(q, s, y_{\text{guidance}}) \quad \text{for } i=1,\dots,N_s
\end{equation}
...
\paragraph{Analyzer.}
Given an execution trace $\tau$, the current tool $\tilde{t}$, and the attack objective $y_{\text{guidance}}$, the analyzer $\mathcal{A}$ generates a structured feedback signal $d$:
\begin{equation}
    d = \mathcal{A}(\tau, \tilde{t}, y_{\text{guidance}})
    \label{eq:analyzer}
\end{equation}
The analyzer acts as a diagnostic critic, and for different attack scenarios it adopts two feedback strategies:
\begin{itemize}[leftmargin=*,nosep]
\item \textbf{Scalar objectives} (the C-DoS attack): focus on \textit{impact amplification}. Even when the tool is invoked, $\mathcal{A}$ suggests ways to raise cost (such as increasing loop depth or output verbosity).
\item \textbf{Binary objectives} (Information Exfiltration, Environment Integrity, Reasoning Derailment): perform \textit{failure diagnosis}. $\mathcal{A}$ pinpoints where the agent diverged from the intended path and proposes corrections to redirect it toward the malicious target.
\end{itemize}
...
\Require Budgets $N_{\text{attr}}, N_{\text{total}}$, Rollouts $K_{\text{roll}}$, Iterations $T$, Elite size $k$
\Ensure Optimized malicious tool $\tilde{t}^*$

\Statex \textbf{Initialization:} $\mathcal{C}_{\text{meta}} \gets \emptyset$, $\mathcal{P} \gets \emptyset$

\Statex \hrulefill
\Statex \textbf{Phase I: Attraction (Metadata Optimization)}
\For{strategy $s \in \mathcal{S}$}
    \State $m \gets N_{\text{attr}} / |\mathcal{S}|$
    \State Generate $\{(n_i, d_i)\}_{i=1}^m \sim \mathcal{G}(q, s, y^*)$
    \For{each $(n, d)$}
        \State $J_{\text{attr}} \gets \frac{1}{K_{\text{roll}}} \sum_{j=1}^{K_{\text{roll}}} \indicator[\tilde{t} \in \tau_j]$ \textbf{with} $\tau_j \sim \pi(q, (n, d, \emptyset))$
...
\For{each $(n, d, J) \in \mathcal{E}_{\text{meta}}$}
    ...
    \State Generate $\{o_i\}_{i=1}^{N_p} \sim \mathcal{G}(q, n, d, y^*)$
    \For{each $o_i$}
...
\For{epoch $t = 1$ to $T$}
    \State $\mathcal{E}_{\text{tools}} \gets \text{TopK}(\mathcal{P}, k)$
    \For{each $(\tilde{t}, \_) \in \mathcal{E}_{\text{tools}}$}
        \State $\tau \gets \text{Execute}(\pi, q, \tilde{t})$
        \State $\delta \gets \mathcal{A}(\tau, \tilde{t}, y^*)$
        \State $\tilde{t}' \gets \mathcal{O}(\tilde{t}, \delta, y^*)$
        \State $\tau' \gets \text{Execute}(\pi, q, \tilde{t}')$
        \State $\mathcal{P} \gets \mathcal{P} \cup \{(\tilde{t}', f(\tau'))\}$
    \EndFor
\EndFor
