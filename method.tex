...
\begin{equation}
    ...
    \label{eq:objective}
\end{equation}
where $\mathcal{J}(\tau)$ scores how effectively execution trace $\tau$ achieves the attack objective, as detailed in Section~\ref{sec:fitness}. The expectation accounts for the stochastic nature of agent decision-making and tool responses.
...
\begin{equation}
    \mathcal{J}(\tau) = \underbrace{\mathbb{I}[\tilde{t} \in \tau]}_{\text{Attraction}} \cdot \underbrace{\mathcal{J}_{\text{man}}(\tau, o_{\text{adv}})}_{\text{Manipulation}}
    \label{eq:decomposition}
\end{equation}
where the selection indicator $\mathbb{I}[\tilde{t} \in \tau]$ ensures fitness is zero when the malicious tool is not invoked, and $\mathcal{J}_{\text{man}}(\tau, o_{\text{adv}})$ quantifies the attack effectiveness when the tool is selected.

This motivates our two-phase optimization approach: Phase~I optimizes $(n, d)$ to maximize the probability of the agent selecting the malicious tool, while Phase~II optimizes $o_{\text{adv}}$ to maximize manipulation effectiveness based on the $(n, d)$ derived from Phase~I.

\subsection{Phase I: Strategy-Guided Attraction Optimization}

The first phase generates tool metadata $(n, d)$ that maximizes the likelihood of agent selection. We employ five persuasion strategies to systematically explore the search space:
...
\begin{itemize}[leftmargin=*,nosep]
    ...
    \item \textbf{Security} ($s_{\text{sec}}$): Emphasizes safety and encryption.
\end{itemize}
...
\begin{equation}
\mathrm{Score}_{\mathrm{attr}}(n, d) = \frac{1}{K} \sum_{k=1}^{K} \mathbb{1}[\tilde{t} \in \tau_k]
\end{equation}
...
\begin{equation}
f(\tau) = \mathbb{1}[\tilde{t} \in \tau] \cdot \left( T_{\mathrm{in}}(\tau) + 5 \cdot T_{\mathrm{out}}(\tau) \right)
\end{equation}
where $T_{\mathrm{in}}$ and $T_{\mathrm{out}}$ denote input and output token counts. Output tokens are weighted 5× higher to reflect their greater computational cost.
...
\begin{equation}
f(\tau) = \mathbb{1}[\tilde{t} \in \tau] \cdot (1 - r_{\mathrm{success}})
\end{equation}
